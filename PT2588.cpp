...
PT2588::PT2588(boolean a, boolean b){
...
byte PT2588::clear_resister(){
  Wire.beginTransmission(_adr);
  Wire.write(0B11000000);              // clear resister
  return Wire.endTransmission();
}

byte PT2588::mute(boolean m,byte cha){
  byte c = ch(cha);
  byte mu;
...
byte PT2588::setvolume(byte v, byte cha){
  clear_resister();
  byte one,ten = 0;
  if(v>79) v=79;
  v =79-v;
...
byte PT2588::setvolume_per(byte v, byte cha){
  clear_resister();
  byte one,ten = 0;
  if(v>100) v=100;
  v = map(v,0,100,0,79);
